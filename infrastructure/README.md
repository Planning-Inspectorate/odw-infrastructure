...
| <a name="input_datalake_location"></a> [location](#input\_datalake_location) | The short-format Azure region into which the data lake will be deployed. This is needed because Disaster Recovery failover results in the datalake being in a different region to the rest of the ODW. By default, this matches the value of `location`, but needs to be updated after DR failover as required | `string` | n/a | yes |
