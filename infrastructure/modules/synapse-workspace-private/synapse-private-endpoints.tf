...
resource "azurerm_synapse_managed_private_endpoint" "synapse_mpe_appeals_bo_sb" {
  ...
  fully_qualified_domain_names = var.appeals_bo_sb_fqdn != null ? [var.appeals_bo_sb_fqdn] : null
...
resource "azurerm_synapse_managed_private_endpoint" "mpesc_prod_sql" {
  ...
  subresource_name             = "sqlServer"
  fully_qualified_domain_names = ["pins-sql-peas-primary-prod.database.windows.net"]
...
resource "azurerm_synapse_managed_private_endpoint" "crown_sql" {count = var.environment == "build" ? 0 : 1
